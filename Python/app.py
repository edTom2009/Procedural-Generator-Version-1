...
def generateOctaves(lacunarity, step, gradVectGrid):
    ...
    octaveGrid = np.empty((maxHeight, maxWidth), dtype=float) #creates array for based on map size
    ...
    for j in range(maxHeight):
        ...
        for i in range(maxWidth):
            ...
            #get the gradient vectors for the four corners of the cell
            ...
            #calculate the local coordinates of the pixel within the cell
            local_x = (x % CELLSIZE) / CELLSIZE
            local_y = (y % CELLSIZE) / CELLSIZE

            #calculate the distance vectors from the pixel to each corner of the cell
            ...
            #calculate the dot products of the distance vectors and the gradient vectors
